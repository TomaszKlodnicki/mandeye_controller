...
void* GetLibHandle(const std::string& libPath)
{
	...
	std::vector<std::filesystem::path> paths = {"/usr/local/lib/", "/opt/mandeye/", parent_path};
...
std::shared_ptr<ClientType> make_dynamic_client(const std::string& libPath, const std::string& createSym, const std::string& destroySym)
{
...
BaseLidarClientPtr createLidarClient(const std::string& lidarType, const nlohmann::json& config)
{
	if(lidarType == "LIVOX_SDK2")
	{
		try
		{
			return make_dynamic_client<BaseLidarClient>("liblivox2_lib.so", "create_livox_client", "destroy_livox_client");
		}
	...
	else if(lidarType == "OUSTER")
	{
		try
		{
			return make_dynamic_client<BaseLidarClient>("libouster_lib.so", "create_ouster_client", "destroy_ouster_client");
		}
		catch(const std::exception& e)
		{
	...
	else if(lidarType == "SICK")
	{
		try
		{
			return make_dynamic_client<BaseLidarClient>("libsick_lib.so", "create_sick_client", "destroy_sick_client");
		}
		catch(const std::exception& e)
		{
